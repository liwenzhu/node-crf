...
//    var gradients = getGradients(X, feature, expectations);
    var result = {};
    result.cost = 0;
    //    result.gradients = $M(gradients);
    result.gradients = $M(thetas);
    return result;
}

function getGradients (X, featrue, expectations) {
    var scores = [];
    X = $M(X)
    var Y = $M(X)
    
    // TODO compute all gradients for every feature
    for (var i = 0, len = feature.size(); i < len; i++) {
	// TODO compute scores for every feature function
...
function calcExpectation (X, y, feature, wordLattice, options) {
    ...
    console.log('feature size:', feature.size());
    // TODO warning: this loop take 95% running time
    // TODO warning: this loop take 95% running time
	// TODO combine feature and word lattice
	
	///////
    var expectation = 0.0;
    for (var i = 1, len=X.length; i <= len; i++) {
	a = getAlpha(alpha, i-1);
	b = getBeta(beta, i);
	
	for (var k = 0, flen = feature.size(); k < flen; k++) {
	    m = $M(wordLattice[i-1]);
	    expectation += a.x(m).x(b).e(1,1);
	}
