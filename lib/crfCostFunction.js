...
function forwardbackward (lattice, labelSize) {
	...
	for (j = 0; j < labelSize; j++) {
	    // compute beta[i,j]
	    //var beta = calcBeta(lattice, labelSize);
	}
    }
...
function calcAlpha (lattice, labelSize) {
    ...
    for (var i = 0; i < len; i++) {
	// compute alpha
	// TODO should check the value is correct later
	for (var j = 0; j < labelSize; j++) {
	    alpha[i][j] = logsumexp(lattice[i], alpha, i, j);
	}
    }
};

function calcBeta (lattice, labelSize) {
    // TODO compute beta
};
